...
public interface AuditRecordRepository extends Neo4jRepository<AuditRecord, Long> {
	...
	@Query("MATCH (ar:AuditRecord) MATCH (page:PageState) WHERE id(ar)=$audit_record_id AND id(page)=$page_state_id MERGE (ar)-[h:FOR]->(page) RETURN ar")
	...
	@Query("MATCH (audit:DomainAuditRecord)-[:HAS]->(d:Domain{url: $url}) RETURN audit ORDER BY audit.created_at DESC LIMIT 1")
	public Optional<AuditRecord> getMostRecentAuditRecordForDomain(@Param("url") String url);
	
	@Query("MATCH (audit:DomainAuditRecord)-[:HAS]->(d:Domain) WHERE id(d)=$id RETURN audit ORDER BY audit.created_at DESC LIMIT 1")
	...
	@Query("MATCH (ar:DomainAuditRecord) MATCH (map:DomainMap) WHERE id(ar)=$audit_record_id AND id(map)=$domain_map_id MERGE (ar)-[:CONTAINS]->(map) RETURN ar")
